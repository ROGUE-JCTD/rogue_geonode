...
from rogue_geonode.middleware import ROGUELoginRequiredMiddleware


class RogueTestCase(TestCase):
    """
    Tests for Rogue.
    """
    ...
    def test_rogue_login_middleware(self):
        """
        Tests the Rogue authentication middleware.
        """

        middleware = ROGUELoginRequiredMiddleware()
        ...
        # The middleware should return None when an authenticated user attempts to visit a white-listed url.
        for path in black_list:
            request.path = path
            response = middleware.process_request(request)
